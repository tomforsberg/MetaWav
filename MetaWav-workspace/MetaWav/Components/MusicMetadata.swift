...
struct ID3Genre {
    ...
    static func code(for name: String) -> Int? {
        return genres.first { $0.value.lowercased() == name.lowercased() }?.key
    }
    
    static var allGenres: [(id: Int, name: String)] {
        return genres.map { (id: $0.key, name: $0.value) }.sorted { $0.name < $1.name }
    }
